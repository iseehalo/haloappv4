...
export default function ProductPage() {
  ...
  const handlePurchase = async () => {
    ...
    // Call Supabase RPC function buy_product
    const { data, error } = await supabase.rpc('buy_product', {
      user_uuid: supabase.auth.user()?.id, // ensure user is logged in
      product_uuid: product.id,
      quantity: 1,
      shipping_address: 'Default Address', // Replace with actual user address input
    });
